# Деплой Query Rewrite На Stage

## Задача

Раскатить на stage изменения с включаемым `query rewrite` для RAG.

## Что задеплоено

- Коммит: `6227243`
- Сообщение коммита: `Add query rewrite toggle for RAG retrieval`
- Ветка: `develop`
...
- `origin` локально настроен на `https://github.com/...`, поэтому push выполнен по SSH без изменения продуктового кода
