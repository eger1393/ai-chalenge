...
import type { AiReviewClient, DiffProvider, OutputWriter, ReviewContextInput, ReviewContextResolver, ReviewPublisher } from "./ports.js";
...
export class ReviewRunner {
  ...
  async run(input: ReviewContextInput): Promise<void> {
    ...
    const publication = { context, result, model, profileName, diff };

    for (const publisher of this.options.publishers) {
      await publisher.publish(publication);
    }

    this.options.output.write({ model, profile: profileName, prNumber: context.prNumber, ...result });
  }
}
