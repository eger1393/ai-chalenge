...
1. **Автодетект** — по ключевым словам и контексту запроса:
   ...
   - Деплой, задеплоить, выкатить, обновить на сервере, пуш на прод, релиз → **Деплой**
...
| Деплой      | Выкатка изменений на сервер                   |
...
Все детали деплоя (адреса, порты, процедура, скрипты) описаны в файле `./deploy.md`. Перед выполнением деплоя ОБЯЗАТЕЛЬНО прочитать этот файл через Read tool — он содержит актуальные инструкции.

### Deploy Workflow (STRICT — нельзя игнорировать)

#### Стадии

1. **Pre-check** — проверка готовности к деплою
2. **Commit & Push** — коммит и пуш в remote
3. **Deploy** — выполнение деплоя на сервере
4. **Verify** — проверка что всё работает
5. **Report** — отчёт о деплое
6. **Done** — задача завершена

#### Разрешённые переходы

```
Pre-check  -> Commit & Push
Pre-check  -> Report          (не готов к деплою — отчёт с причиной)
Commit & Push -> Deploy
Deploy     -> Verify
Deploy     -> Report          (деплой упал — отчёт с ошибкой)
Verify     -> Report
Verify     -> Deploy          (откат, повторный деплой)
Report     -> Done
```

#### Pre-check — что проверяем

1. Прочитать `./deploy.md` через Read tool
2. Запустить `git status` — убедиться что нет незакоммиченных изменений, или закоммитить их
3. Запустить `npm run build` в backend и frontend — убедиться что билд проходит
4. Спросить через `AskUserQuestion`: "Что деплоим? Кратко опиши изменения" (если не указано в запросе)

#### Commit & Push

1. Если есть незакоммиченные изменения — закоммитить (спросить пользователя о commit message)
2. Запустить `git push origin develop`
3. Убедиться что push прошёл без ошибок

#### Deploy — выполнение на сервере

Все параметры подключения и команды берутся из `./deploy.md`. Общий порядок:

1. Подключиться к серверу через `mcp__ssh-mcp__ssh_open_session` (параметры из deploy.md)
2. Проверить что .env на сервере содержит все нужные переменные (если добавлялись новые)
3. Запустить скрипт деплоя через `mcp__ssh-mcp__proc_exec` (команда из deploy.md)
4. Дождаться завершения, проверить exit code

#### Verify — проверка

1. Через `mcp__ssh-mcp__proc_exec` проверить `docker compose ps` — все контейнеры Up
2. Через `mcp__ssh-mcp__proc_exec` проверить `docker compose logs --tail=20` — нет ошибок в логах
...
- Проблемы (если были)
- Статус: Deployed / Failed / Rolled Back
