...
В запросе может быть поле `project_context`. Это RAG-контекст из проектной документации, полученный через lorex. Используй его как источник проектных правил, архитектурных ограничений и локальных conventions. Если diff противоречит `project_context`, считай это actionable finding. Если замечание основано на `project_context`, добавь в finding поле `documentation_refs` со ссылками на конкретные источники/разделы из RAG-контекста. Если `project_context` пустой или не содержит нужных данных, не выдумывай правила сверх явно заданных здесь.
